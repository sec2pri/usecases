...
##Collected Release Date for comparisons later:
get_release_date <- function(study_ids) {
  
  ##Create dataframe to collect data in:
  release_files <- data.frame(
    study_id = character(),
    study_release = character(),
  ...
  for (study_id in study_ids) {
    ...
    if (status_code(response) == 200) {
      ...
      study_release <- files_info$isaInvestigation$publicReleaseDate
      ...
      release_files <- rbind(release_files, newdata)
      
    } else {
      warning("Failed to retrieve files list for study: ", study_id)
    }
...
Release_Files_Dates <- get_release_date(vStudyIds)
  ...
  for (maf_filename in maf_filenames) {
    ...
    if (status_code(response) == 200) {
      ...
      if (!is.na(response_content)) {
        # Read tsv content
        tsv_df <- read_tsv(file = I(response_content), show_col_types = FALSE, col_types = cols(.default = col_guess()))
      ...
      message( maf_filename, ": Error 404 Not Found is an HTTP status code indicating that the server could not find the requested page. This error typically occurs when the URL is incorrect, the page has been moved or deleted, or there are issues with the server configuration.")
    } 
    else {
...
```
##Clean ID list before comparison:
```{r}
##Clean up dataframe IDs with | symbol for multiple IDs
df_split <- finalDF_sanityCheck %>%
  separate_rows(IDs, sep = "\\|")
##Remove "empty" IDs e.g. '-'
df_clean <- df_split[!grepl("-", df_split$IDs, fixed = TRUE), ]
##Remove IDs that do not start with ChEBI (e.g. '12345' could be ChEBI but provenance not clear to include in analysis)
df_filtered <- df_clean[grepl("^CHEBI", df_clean$IDs), ]
```


##Compare final Dataframe to list of secondary ChEBIs
```{r}
##Some stats on included data:
totalincludedIDs <-length(unique(finalDF_sanityCheck$studyName))
print(paste("Total number of study IDs with ChEBI IDs: ",totalincludedIDs))
includedIDs <-length(unique(df_filtered$studyName))
print(paste("Number of study IDs with ChEBI IDs: ",includedIDs))
notIncludedStudies <- length(unique(setdiff(finalDF_sanityCheck$studyName, df_filtered$studyName)))
print(paste("Number of study IDs not included in analysis: ",notIncludedStudies))

##Read in ChEBI sec and pri ID data
secChEBI <- read.csv(paste0(workingDirectory, '/ChEBI_secID2priID.tsv'), header = TRUE, sep = "\t")

##Read in ChEBI all pri ID data
totalChEBI <- read.csv(paste0(workingDirectory, '/ChEBI_priIDs.tsv'), header = TRUE, sep = "\t")

#Compare unique overlap with sec and pri IDs and print results
print(paste("Total number of unique ChEBI IDs in Metabolights: ", length(unique(df_filtered$IDs))))
overlapSecIDs <- intersect(unique(df_filtered$IDs),secChEBI$secondaryID)
print(paste("Total number of unique secondary IDs in Metabolights: ", length(overlapSecIDs)))

##Percentage of secondary IDs over total of unique IDs:
print(paste("Percentage of secondary IDs in total:", (length(overlapSecIDs)*100)/length(unique(df_filtered$IDs))))

##Check in how many study IDs the secondary IDs occur, and if some studies have a high percentage of sec. IDs:
# Filter rows where column 2 matches IDs
df_studiesSec <- df_filtered[df_filtered$IDs %in% secChEBI$secondaryID, ]
print(paste("Total number of studies in Metabolights including a secondary ID: ", length(unique(df_studiesSec$studyName))))

##Percentage of studies including secondary IDs:
print(paste("Percentage of studies in Metabolights including a secondary ID:", (length(unique(df_studiesSec$studyName))*100)/length(unique(df_filtered$studyName))))


##Analyze how many studies have more than 1 secondary ID
df_studiesSec <- unique(df_studiesSec)
itemCountsSec <- table(df_studiesSec$studyName)
MoreThanOne_SecID <- sum(itemCountsSec > 1)

##To visualise distribution:
dat <- as.data.frame(itemCountsSec)
print(paste("Average number of secondary IDs in studies:", mean(dat$Freq)))
print(paste("Studies with more than 1 Secondary IDs:", sum(dat$Freq > 1)))
print(paste("Highest number of secondary IDs within one study:", max(dat$Freq)))

##Check which IDs are not on list primary and secondary but found in datasets (for validation of results):
#overlapPriIDs <- intersect(unique(df_filtered$IDs),totalChEBI$primaryID)
#print(paste("Total number of unique primary IDs in Metabolights: ", length(overlapPriIDs)))

#combinedSecandPriIDs <- unique(c(totalChEBI$primaryID, secChEBI$secondaryID))
#non_overlap <- df_filtered[!df_filtered$IDs %in% combinedSecandPriIDs, ]
#print(paste("IDs not matching pri and sec list for studies in Metabolights: ", length(unique(non_overlap$IDs))))
